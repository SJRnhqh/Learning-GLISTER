...
from models.set_function_grad_computation_taylor import (
    SetFunctionLoader_2 as SetFunction,
)
from models.set_function_grad_computation_taylor import (
    WeightedSetFunctionLoader as WtSetFunction,
)
import math
...
def perform_knnsb_selection(datadir, dset_name, budget, selUsing):
    ...
    idxs_knnsb = np.genfromtxt(
        indices_file, delimiter=",", dtype=int
    )  # since they are indices!
    return idxs_knnsb
...
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=trn_batch_size, shuffle=False, pin_memory=True
)
...
testloader = torch.utils.data.DataLoader(
    testset, batch_size=tst_batch_size, shuffle=False, pin_memory=True
)

trainset_idxs = np.array(trainset.indices)
batch_wise_indices = trainset_idxs[
    list(BatchSampler(SequentialSampler(trainset_idxs), 1000, drop_last=False))
]
cnt = 0
for batch_idx in batch_wise_indices:
    inputs = torch.cat([fullset[x][0].view(1, -1) for x in batch_idx], dim=0).type(
        torch.float
    )
...
def train_model_craig(start_rand_idxs, bud):
    ...
    setf_model = CRAIG(
        device, model, trainset, N_trn=N, batch_size=1000, if_convex=False
    )
    ...
    for i in range(0, num_epochs):
        ...
        batch_wise_indices = list(
            BatchSampler(RandomSampler(actual_idxs), trn_batch_size, drop_last=False)
        )
        subtrn_loss = 0
        for batch_idx in batch_wise_indices:
            inputs = torch.cat(
                [
                    fullset[actual_idxs[x]][0].view(
                        -1,
                        1,
                        fullset[actual_idxs[x]][0].shape[1],
                        fullset[actual_idxs[x]][0].shape[2],
                    )
                    for x in batch_idx
                ],
                dim=0,
            ...
            loss = torch.dot(
                torch.from_numpy(batch_gammas).to(device).type(torch.float), losses
            )
...
def random_greedy_train_model_online_taylor(start_rand_idxs, bud, lam):
    ...
    for i in range(0, num_epochs):
        ...
        if ((i + 1) % select_every) == 0:
            ...
            clone_dict = copy.deepcopy(model.state_dict())
            print("selEpoch: %d, Starting Selection:" % i, str(datetime.datetime.now()))
            subset_idxs, grads_idxs = setf_model.naive_greedy_max(
                int(lam * bud), clone_dict
            )
            rem_idxs = list(set(total_idxs).difference(set(subset_idxs)))
            subset_idxs.extend(
                list(
                    np.random.choice(rem_idxs, size=int((1 - lam) * bud), replace=False)
                )
            )
            idxs = subset_idxs
            print(
...
def facloc_reg_train_model_online_taylor(start_rand_idxs, facloc_idxs, bud, lam):
    ...
    cmbloader = torch.utils.data.DataLoader(
        cmb_set, batch_size=1000, shuffle=False, pin_memory=True
    )
...
(
    ...
    craig_full_trn_loss,
    craig_val_losses,
    craig_substrn_losses,
    craig_fulltrn_losses,
    craig_subset_idxs,
    craig_time,
...
(
    ...
    one_val_tstacc,
    one_val_subtrn_acc,
    one_val_full_trn_acc,
    one_val_valloss,
    one_val_tstloss,
    one_val_subtrnloss,
    one_val_full_trn_loss,
    one_fval_val_losses,
    one_fval_substrn_losses,
    one_fval_fulltrn_losses,
    one_subset_idxs,
    one_step_time,
...
(
    facloc_reg_val_valacc,
    facloc_reg_val_tstacc,
    facloc_reg_val_subtrn_acc,
    facloc_reg_val_full_trn_acc,
    facloc_reg_val_valloss,
    facloc_reg_val_tstloss,
    facloc_reg_val_subtrnloss,
    facloc_reg_val_full_trn_loss,
    facloc_reg_fval_val_losses,
    facloc_reg_fval_substrn_losses,
    facloc_reg_fval_fulltrn_losses,
    facloc_reg_subset_idxs,
    facloc_one_step_time,
...
(
    ...
    mod_val_tstacc,
    mod_val_subtrn_acc,
    mod_val_full_trn_acc,
    mod_val_valloss,
    mod_val_tstloss,
    mod_val_subtrnloss,
    mod_val_full_trn_loss,
    mod_val_val_losses,
    mod_val_substrn_losses,
    mod_val_fulltrn_losses,
    mod_subset_idxs,
    mod_one_step_time,
...
(
    ...
    ol_rand_tstacc,
    ol_rand_subtrn_acc,
    ol_rand_full_trn_acc,
    ol_rand_valloss,
    ol_rand_tstloss,
    ol_rand_subtrnloss,
    ol_rand_full_trn_loss,
    ol_rand_val_losses,
    ol_rand_substrn_losses,
    ol_rand_fulltrn_losses,
    ol_random_run_time,
) = train_model_random_online(start_idxs)
...
(
    rand_reg_val_valacc,
    rand_reg_val_tstacc,
    rand_reg_val_subtrn_acc,
    rand_reg_val_full_trn_acc,
    rand_reg_val_valloss,
    rand_reg_val_tstloss,
    rand_reg_val_subtrnloss,
    rand_reg_val_full_trn_loss,
    rand_reg_fval_val_losses,
    rand_reg_fval_substrn_losses,
    rand_reg_fval_fulltrn_losses,
    rand_reg_subset_idxs,
    random_reg_one_step_time,
...
(
    ...
    facloc_tstacc,
    facloc_subtrn_acc,
    facloc_full_tran_acc,
    facloc_valloss,
    facloc_tstloss,
    facloc_subtrnloss,
    facloc_full_trn_loss,
    facloc_val_losses,
    facloc_substrn_losses,
    facloc_fulltrn_losses,
    facloc_idxs,
    facloc_time,
) = train_model_Facloc(facloc_idxs)
...
plt.figure()
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    one_fval_substrn_losses[plot_start_epoch:],
    "b-",
    label="tay_v=val",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    craig_substrn_losses[plot_start_epoch:],
    "r",
    label="craig",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    rand_substrn_losses[plot_start_epoch:],
    "g-",
    label="random",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    ol_rand_substrn_losses[plot_start_epoch:],
    "g+",
    label="Online random",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    facloc_substrn_losses[plot_start_epoch:],
    "pink",
    label="FacLoc",
)
...
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    rand_reg_fval_substrn_losses[plot_start_epoch:],
    "k-",
    label="rand_reg_tay_v=val",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    facloc_reg_fval_substrn_losses[plot_start_epoch:],
    "y",
...
plt.title(
    "Subset Training Loss vs Epochs "
    + data_name
    + "_"
    + str(fraction)
    + "_"
...
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    one_fval_fulltrn_losses[plot_start_epoch:],
    "b-",
    label="tay_v=val",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    craig_fulltrn_losses[plot_start_epoch:],
    "r",
    label="craig",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    rand_fulltrn_losses[plot_start_epoch:],
    "g-",
    label="random",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    ol_rand_fulltrn_losses[plot_start_epoch:],
    "g+",
    label="Online random",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    facloc_fulltrn_losses[plot_start_epoch:],
    "pink",
    label="FacLoc",
)
...
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    rand_reg_fval_fulltrn_losses[plot_start_epoch:],
    "k-",
    label="rand_reg_tay_v=val",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    facloc_reg_fval_fulltrn_losses[plot_start_epoch:],
    "y",
    label="facloc_reg_tay_v=val",
)
...
plt.title(
    "Full Training Loss vs Epochs " + data_name + "_" + str(fraction) + "_" + "val=VAL"
)
...
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    one_fval_val_losses[plot_start_epoch:],
    "b-",
    label="tay_v=val",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    craig_val_losses[plot_start_epoch:],
    "r",
    label="craig",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    rand_val_losses[plot_start_epoch:],
    "g-",
    label="random",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    ol_rand_val_losses[plot_start_epoch:],
    "g+",
    label="Online random",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    facloc_val_losses[plot_start_epoch:],
    "pink",
    label="FacLoc",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    rand_reg_fval_val_losses[plot_start_epoch:],
    "k-",
    label="rand_reg_tay_v=val",
)
plt.plot(
    np.arange(plot_start_epoch, num_epochs),
    facloc_reg_fval_val_losses[plot_start_epoch:],
    "y",
    label="facloc_reg_tay_v=val",
)
...
plt.title(
    "Validation Loss vs Epochs " + data_name + "_" + str(fraction) + "_" + "val=VAL"
)
...
print(
    "*| random regularized Taylor after training               |",
    rand_reg_val_valacc,
    "  | ",
    rand_reg_val_tstacc,
    " |",
...
print(
    "*| facloc regularizec Taylor after training               |",
    facloc_reg_val_valacc,
    "  | ",
    facloc_reg_val_tstacc,
    " |",
...
print(
    "*| Taylor after training               |", mod_one_step_time, "  | ", file=logfile
)
print(
    "*| random regularized Taylor after training               |",
    random_reg_one_step_time,
...
print(
    "=========Online Selection Taylor with Validation Set===================",
    file=logfile,
)
...
print(
    "=========Random Regularized Online Selection Taylor with Validation Set===================",
    file=logfile,
)
...
print(
    "*Taylor v=VAL Full Trn Data LOSS:", rand_reg_fval_fulltrn_losses[-1], file=logfile
)

print(
    "=========Facility Location Loss regularized Online Selection Taylor with Validation Set===================",
    file=logfile,
)
...
print(
    "*Taylor v=VAL Full Trn Data LOSS:",
    facloc_reg_fval_fulltrn_losses[-1],
    file=logfile,
)

print(
    "=========Online Selection Taylor after model training with Validation Set===================",
    file=logfile,
)
